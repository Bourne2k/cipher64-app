...
// Strict Specta bindings
import { type EngineOptions, events, type GoMode } from "@/bindings/generated";
...
function EvalListener() {
  ...
  if (loadableEngines.state === "hasData") {
    return loadableEngines?.data?.map((e: any) => (
      <EngineListener
        key={e.name}
        engine={e}
    ...
    () => {
      if (settings.enabled) {
        if (isGameOver) {
          ...
          if (chess960 && !options.find((o: any) => o.name === "UCI_Chess960")) {
